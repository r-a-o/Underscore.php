...
function Underscore($item=null) {
  return new _Underscore($item);
}

class Underscore {

  public static function __callStatic($method, $args) {
    $__ = Underscore($args[0]);
...
class _Underscore {
  ...
  public function inject($iterator, $memo=null) {return $this->reduce($iterator, $memo);}
  ...
  public function findWhere($properties) {
    ...
    return $this->_result($collects[0] ? $collects[0] : null);
  }
  ...
  // TODO must have $iterator as underscore.js?
  public function all($iterator=null) {return $this->every($iterator);}
  public function every($iterator=null) {
    if (is_null($iterator)) {
  ...
  public function any($iterator=null) {return $this->some($iterator);}
  public function some($iterator=null) {
    if (is_null($iterator)) {
      $iterator = function($v) {return $v;};
    }
  ...
  public function invoke($method_name /* $arguments*/ ) {
  ...
  public function countBy($iterator) {
    ...
    foreach ($groups as $group => $g) {
      $counts[$group] = count($g);
    }

    return $this->_result($counts);
  }
...
class ____ {
  ...
  public function __construct($item=null, $chained=false) {
    $this->_wrapped = $item;
    $this->_chained = $chained;
  }
  ...
  public function first($n=null) {
    $collection = self::_collection($this->_wrapped);
  ...
  public function initial($collection=null, $n=null) {
    ...
    $__ = new self;
    return self::_wrap($__->first($first_index));
  }
  ...
  public function uniqueId($prefix=null) {
    ...
    return (is_null($prefix)) ? self::_wrap($this->_uniqueId) : self::_wrap($prefix . $this->_uniqueId);
  }
  ...
  public static function __callStatic($name, $arguments) {
    return call_user_func_array($this->_mixins[$name], $arguments);
  }
  ...
  public function templateSettings($settings=null) {
    $_template_settings = $this->_template_settings;
  ...
  public function memoize($function=null, $hashFunction=null) {
    ...
    return self::_wrap(function() use ($function, &$self, $hashFunction) {
      ...
      if(!array_key_exists($key, $self->_memoized)) {
        $self->_memoized[$key] = call_user_func_array($function, $args);
      }
  ...
  public function throttle($function=null, $wait=null) {
    ...
    return self::_wrap(function() use ($function, $wait, &$self) {
      ...
      $ready_to_call = (!array_key_exists($key, $self->_throttled) || $microtime >= $self->_throttled[$key]);
      if($ready_to_call) {
        $next_callable_time = $microtime + ($wait / 1000);
        $self->_throttled[$key] = $next_callable_time;
  ...
  public function once($function=null) {
    ...
    return self::_wrap(function() use ($function, &$self) {

      // Try using var_export to identify the function
      $key = md5(var_export($function, true));
      if(!array_key_exists($key, $self->_onced)) {
        $self->_onced[$key] = call_user_func_array($function, func_get_args());
      }

      return $self->_onced[$key];
    });
  }
